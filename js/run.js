...
function renderCurrentStep(s) {
    ...
    const endIn = endItems(s).map((e, i) => {
        // 투입 Lot은 계보에 이미 있다. 다시 치게 하지 않고 채워 준다.
        if (e.isLot) {
            const known = fromLotNo();
            return `<div class="in-row known">
                      <div class="in-name">${esc(e.item)}</div>
                      <input type="text" class="in-val mono" id="in-end${i}"
                             value="${esc(known)}" placeholder="번호를 적어 주세요" autocomplete="off">
                      <div class="in-unit"></div>
                      <div class="in-plan">${known
                          ? '계보에서 가져왔습니다 — 다르면 고치세요'
                          : '계보가 없어 비어 있습니다'}</div>
                    </div>`;
        }
        return `<div class="in-row measure">
                  <div class="in-name">${esc(e.item)}</div>
...
function endItems(s) {
    ...
    // 이름에 「Lot」이 들어간 항목은 잰 값이 아니라 번호다.
    // 단위(kg)를 붙이면 안 되고, 「계량값」이라고 물어서도 안 된다.
    const asLot = n => /lot/i.test(n);

    const names = String(s.endInput.item).split('·').map(x => x.trim()).filter(Boolean);
    if (names.length <= 1) {
        const n = s.endInput.item;
        return [{ item: n, base: s.endInput.base, unit: asLot(n) ? '' : unit, isLot: asLot(n) }];
    }
    return names.map(n => ({ item: n, base: null, unit: asLot(n) ? '' : unit, isLot: asLot(n) }));
}

/** 계보에 적힌 투입 Lot 번호 — 'DPS426-0831-A08 / 합성' 에서 번호만 */
function fromLotNo() {
    const f = (run.lot && run.lot.from) || '';
