...
class GoogleSheetsManager {
    ...
    // 고치기용 — 시트 줄 번호가 붙은 날것. 캐시하지 않는다(고치는 중에 옛것을 보면 안 된다).
    async getRecipeRows(product, process) {
        if (!this.webAppUrl) throw new Error('웹앱 URL이 설정되지 않았습니다.');
        const params = new URLSearchParams({ action: 'getRecipeRows', product, process });
        const result = await this._jsonpRequest(params, TIMEOUT_READ);
        if (result && result.success) return result.data || [];
        throw new Error(result?.error || '레시피를 불러오지 못했습니다');
    }

    async updateRecipeRow(product, process, row, patch) {
        if (!this.webAppUrl) throw new Error('웹앱 URL이 설정되지 않았습니다.');
        const params = new URLSearchParams({
            action: 'updateRecipeRow', product, process,
            row: String(row), patch: JSON.stringify(patch || {})
        });
        const result = await this._jsonpRequest(params, TIMEOUT_WRITE);
        if (result && result.success) {
            console.log(`✏️ 레시피 ${product}/${process} ${row}행 — ${result.data?.changed || 0}칸`);
            return result.data;
        }
        throw new Error(result?.error || '레시피를 고치지 못했습니다');
    }
