...
/* ========================================
   모드 전환 — 공정 입력 · 설비 및 장비 점검
   접속하면 갈림길 화면이 먼저 뜨고, 고른 모드는 새로고침을 넘어 기억된다.
   (점검 완료 후 goBackToMain()이 새로고침을 하는데, 그때 갈림길로
    되돌아가 버리면 팀을 다시 고르게 되어 한 번 더 누르게 된다)
   ======================================== */
const MODE_KEY = 'facsys_mode';

function getMode() {
    // 사생활 보호 모드 등에서 sessionStorage 자체가 막힐 수 있다.
    // 그래도 화면은 돌아야 하므로 실패하면 '모드 없음'으로 본다.
    try { return sessionStorage.getItem(MODE_KEY) || ''; } catch { return ''; }
}
...
// 헤더의 「점검 알림」·「종합 현황」은 설비점검에서만 쓸모가 있다.
// 공정 입력 화면에 그대로 두면 눌렀을 때 엉뚱한 곳으로 간다.
function applyHeaderForMode(mode) {
    const visible = (mode === 'inspection') ? '' : 'none';
    ['btn-alarm', 'btn-overview'].forEach(id => {
...
const ALL_SCREENS = [
    'mode-screen', 'process-screen', 'product-entry-screen', 'main-screen',
    'dashboard-screen', 'overview-hub-screen',
    'repair-status-screen', 'all-teams-dashboard-screen', 'inspection-screen'
];
...
function showScreen(id, mode) {
    ...
    applyHeaderForMode(mode);
    window.scrollTo({ top: 0, behavior: 'instant' });
}

function showModeScreen() { showScreen('mode-screen', ''); }

function enterProcess() {
    setMode('process');
    showScreen('process-screen', 'process');
}

function enterInspection() {
    setMode('inspection');
    showScreen('main-screen', 'inspection');
}

// 로고와 「처음 화면」 — 고른 모드를 지우고 상태까지 깨끗이 비운다.
function goToStart() {
    setMode('');
    setTimeout(() => { location.reload(); }, 50);
}

// openProductEntry()는 js/process.js에 있다.
// (process.js가 app.js보다 먼저 실행되므로, 여기에 같은 이름을 두면 그쪽이 덮인다)
function openProcessRun() {
    alert('공정 진행 화면은 다음 단계에서 만듭니다.');
}
    ...
    await Promise.all(teams.map(async (team) => {
        ...
        for (const type of types) {
            ...
            try {
                ...
                // 통신 실패를 '미완료'로 보여주면, 점검을 다 해놓고도 안 한 것처럼 읽힙니다.
                // 실제 미완료와 반드시 구분해야 합니다.
                console.warn(`[점검 알림] ${team.sheet}/${type} 조회 실패:`, err?.message);
                loadFailed = true;
                break;
            }
        }
        ...
        if (loadFailed) {
            if (dot) dot.style.backgroundColor = '#9E9E9E';
            if (label) { label.textContent = '확인 실패'; label.style.color = '#9E9E9E'; }
        } else {
...
function openAllTeamsDashboard() {
    ...
    if (allTeamsScreen) { allTeamsScreen.style.display = 'none'; allTeamsScreen.classList.remove('active'); }
    if (repairScreen) { repairScreen.style.display = 'none'; repairScreen.classList.remove('active'); }

    if (!hubScreen) {
        alert('종합 현황 화면을 찾을 수 없습니다.');
        return;
    }

    hubScreen.style.display = 'block';
    hubScreen.style.visibility = 'visible';
    hubScreen.classList.add('active');
    window.scrollTo({ top: 0, behavior: 'instant' });
}

function goBackToOverviewHub() {
    const allTeamsScreen = document.getElementById('all-teams-dashboard-screen');
    const repairScreen = document.getElementById('repair-status-screen');
    if (allTeamsScreen) { allTeamsScreen.style.display = 'none'; allTeamsScreen.classList.remove('active'); }
    if (repairScreen) { repairScreen.style.display = 'none'; repairScreen.classList.remove('active'); }
...
/* ========================================
   팀별 점검 현황 (허브에서 진입)
   ======================================== */
async function openTeamStatusFromHub() {
    const hubScreen = document.getElementById('overview-hub-screen');
    const allTeamsScreen = document.getElementById('all-teams-dashboard-screen');
    const container = document.getElementById('all-teams-dashboard');

    if (hubScreen) { hubScreen.style.display = 'none'; hubScreen.classList.remove('active'); }
    ...
    const teamResults = await Promise.all(teams.map(async (team) => {
        ...
        for (const type of inspectionTypes) {
            try {
                ...
                // 0%가 아니라 '실패'로 구분 (아래 셀 렌더링에서 '—'로 표시)
                rates.push({ percentage: 0, completed: 0, total: 0, status: 'error' });
            }
            completedTasks++;
    ...
    for (const { team, rates } of teamResults) {
        ...
        for (const rate of rates) {
            ...
            if (rate.status === 'error') {
                cell.innerHTML = `<span class="status-badge alarm">—</span><span class="status-sub">불러오기 실패</span>`;
            } else {
...
async function openRepairStatusFromHub() {
    ...
    if (!repairScreen) {
        alert('수리 현황 화면을 찾을 수 없습니다.');
        return;
    }

    repairScreen.style.display = 'block';
    repairScreen.style.visibility = 'visible';
...
async function loadRepairDashboard() {
    ...
    let cloudRecords = [];
    const localRecords = repairTracker.getAllNormalized();
    let loadError = null;

    if (googleSheetsManager.webAppUrl) {
        try {
            const raw = await googleSheetsManager.getRepairRecords();
            cloudRecords = (raw || []).map(RepairTracker.normalizeCost);
        } catch (err) {
    ...
    // 클라우드와 로컬을 합칩니다. (예전에는 둘 중 하나만 골라서, 시트 저장이 성공한
    // 뒤에도 이 기기에만 있는 기록이 통째로 사라졌습니다.)
    // 같은 날짜+팀+장비+유형이면 동일 건으로 보고 클라우드 쪽을 우선합니다.
    const keyOf = (r) => `${r.date}|${r.sheet}|${r.type}|${r.equipment}`;
    const merged = new Map();
    for (const r of localRecords) merged.set(keyOf(r), r);
    for (const r of cloudRecords) merged.set(keyOf(r), r);

    // 오래된 순으로 정렬합니다. 아래 '최근 수리 이력'이 reverse().slice(0,20)으로
    // 최신 20건을 뽑기 때문에 반드시 오름차순이어야 합니다.
    // (백엔드 getRepairRecords는 내림차순으로 주기 때문에, 예전에는 reverse 후
    //  가장 오래된 20건이 '최근 수리 이력'으로 표시되고 있었습니다.)
    const allRecords = [...merged.values()].sort((a, b) => (a.date || '').localeCompare(b.date || ''));

    let dataSource;
    if (cloudRecords.length && localRecords.length) dataSource = 'both';
    else if (cloudRecords.length) dataSource = 'cloud';
    else if (localRecords.length) dataSource = 'local';
    else dataSource = loadError ? 'error' : 'empty';
    ...
    if (allRecords.length === 0) {
        container.innerHTML = `
            <div class="repair-empty">
                ...
                <div class="repair-empty-text">수리 이력이 없습니다</div>
                <div class="repair-empty-sub">점검 시 '수리 진행' 항목을 선택하면 여기에 기록됩니다</div>
            </div>`;
        return;
    }
    ...
    const sourceLabels = {
        cloud: '☁️ Google Sheets',
        local: '📱 이 기기 저장소',
        both: '☁️ Google Sheets + 📱 이 기기 저장소'
    };
    const sourceLabel = sourceLabels[dataSource] || '📱 이 기기 저장소';
    ...
    if (recentRecords.length > 0) {
        ...
        for (const record of recentRecords) {
            ...
            // 금액은 별도 배지로 보여주므로 특이사항 본문에서는 [수리금액:N] 태그를 뺍니다.
            const noteText = (record.notes || '').replace(/\[수리금액:\s*\d+\s*\]\s*/, '').trim();
            html += `
                <div class="repair-history-item">
                    <div class="repair-history-date">${record.date}</div>
                    <div class="repair-history-detail">
                    ...
                    ${noteText ? `<div class="repair-history-notes">${noteText}</div>` : ''}
                </div>`;
        }
        html += '</div></div>';
    }
    ...
    try {
        ...
        const results = await Promise.all(inspectionTypes.map(async (type) => {
            try {
                ...
                // 0%로 표시하면 "아무도 점검 안 함"처럼 보여서 실제와 정반대로 읽힙니다.
                // 불러오기에 실패한 것과 정말 0%인 것은 반드시 구분해서 보여줘야 합니다.
                return { type, completionRate: { percentage: 0, completed: 0, total: 0, status: 'error' } };
            }
        }));
...
async function calculateCompletionRate(inspectionType, explicitSheet) {
    ...
    if (googleSheetsManager.webAppUrl) {
        try {
            // ⚠️ 예전 코드는 6초짜리 타임아웃 Promise를 반복문 '밖에서' 딱 한 번 만들었습니다.
            //    타이머가 1차 시도와 함께 시작되므로, 1차가 5초 걸리면 2차 시도는
            //    1초 만에 타임아웃 났습니다. 사실상 재시도가 없는 것과 같았습니다.
            //    이제 getEquipmentFromSheet 안쪽 타임아웃(TIMEOUT_READ)에 맡기고,
            //    실패한 경우에만 한 번 더 시도합니다.
            let sheetEquipment = null;
            let lastError = null;

            for (let attempt = 0; attempt < 2; attempt++) {
                try {
                    sheetEquipment = await googleSheetsManager.getEquipmentFromSheet(inspectionType, targetSheet);
                    if (sheetEquipment && sheetEquipment.length > 0) break;
                } catch (error) {
                    lastError = error;
                    console.warn(`[완료율] ${targetSheet}/${inspectionType} ${attempt + 1}차 시도 실패:`, error.message);
                }
                if (attempt === 0) await new Promise(resolve => setTimeout(resolve, 1000));
            }

            // 두 번 다 예외로 끝났으면 '0%'가 아니라 '불러오기 실패'로 처리해야 합니다.
            if (!sheetEquipment && lastError) throw lastError;
...
function createChart(type, completionRate) {
    ...
    // 데이터를 못 불러온 경우: 0%가 아니라 '!'로 표시합니다.
    // (0%로 보이면 점검을 다 해놓고도 안 한 것처럼 읽혀서 오히려 위험합니다)
    if (status === 'error') {
        chartItem.innerHTML = `
            <div class="chart-container">
                ...
                <div class="chart-percentage" style="color:#f44336; font-size:1.6rem;">!</div>
            </div>
            <div class="chart-label">${type}</div>
            <div class="chart-count" style="color:#f44336;">불러오기 실패<br>다시 시도해주세요</div>
        `;
        return chartItem;
    }
...
async function loadEquipmentList() {
    ...
    if (googleSheetsManager.webAppUrl) {
        try {
            ...
            // 실패했는데 조용히 넘어가면 모든 장비가 '미점검'으로 보입니다.
            // 이미 점검한 장비를 또 점검하게 만드는 원인이 되므로 사용자에게 알려야 합니다.
    ...
    if (sheetLoadFailed) {
        ...
        warn.style.cssText = 'padding:0.9rem 1rem; margin-bottom:1rem; background:#FFF3E0; border:1px solid #FFCC80; border-radius:8px; color:#E65100; font-size:0.9rem; line-height:1.5;';
        warn.innerHTML = '⚠️ <strong>점검 이력을 불러오지 못했습니다.</strong><br>' +
            '아래 상태 표시가 실제와 다를 수 있습니다. 통신 상태를 확인하고 새로고침해주세요.';
        equipmentItems.appendChild(warn);
    }
        ...
        itemDiv.addEventListener('click', () => {
            ...
            if (itemDiv.classList.contains('completed')) {
                ...
                if (isRepairItem) {
                    ...
                    statusEl.style.backgroundColor = '#42A5F5';
                }
                statusEl.style.color = '#fff';
                const resultText = isRepairItem ? '수리 진행' : '점검완료';
                const existing = inspectionData.inspections.find(r => r.item === item);
                if (existing) { existing.result = resultText; }
                else { inspectionData.inspections.push({ item, result: resultText }); }
            }
            updateInspectionSelectionInfo();
        });
...
async function submitInspection() {
    ...
    for (const equipmentName of equipmentList) {
        ...
        try {
            ...
            inspectionTimeManager.recordInspection(inspectionData.type, equipmentName, sheetName);
            successCount++;
        } catch (error) {
            console.error(`${equipmentName} 저장 실패:`, error);
            failedEquipment.push(equipmentName);
            // 시트 저장에 실패한 건은 로컬 점검 이력에도 남기지 않습니다.
            // (남기면 알람이 잘못 꺼져서 미점검 장비를 놓치게 됩니다.)
        }

        if (hasRepair) {
            // 수리 이력은 시트에 따로 저장하지 않습니다.
            // 백엔드 getRepairRecords()가 팀 시트의 점검 기록에서 점검내용(F열)에
            // '수리 진행'이 들어간 행을 찾아 수리 이력을 만들어내고,
            // 특이사항(G열)의 [수리금액:N] 태그를 파싱해 금액까지 채웁니다.
            // 따라서 위 addInspectionData 한 번이면 시트 쪽은 이미 끝난 상태입니다.
            //
            // 아래 로컬 기록은 오프라인/저장실패 대비용 백업입니다.
            // 대시보드에서 클라우드 기록과 date|sheet|type|equipment 키로 병합됩니다.
            repairTracker.addRecord(
                sheetName, inspectionData.type, equipmentName,
                notesWithCost, inspectionData.repairCost
            );
...
// 첫 화면을 정한다. 새로고침해도 있던 자리로 돌아온다.
(function boot() {
    const mode = getMode();
    if (mode === 'inspection') enterInspection();
    else if (mode === 'process') enterProcess();
    else showModeScreen();
    console.log(`[boot] 시작 화면: ${mode || '갈림길'}`);
})();
