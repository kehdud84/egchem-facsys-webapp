...
class InspectionItemsManager {
    ...
    loadInspectionItems() {
        ...
        const defaultItems = ['일반점검', '정기점검', '외관확인', '소음확인', '수리 진행'];
        return {
            '일일점검': defaultItems,
            '주간점검': defaultItems,
            '월간점검': defaultItems,
...
/* ========================================
   수리 이력 관리 (RepairTracker)
   - 수리 진행 기록 저장 및 팀별 집계
   ======================================== */

class RepairTracker {
    constructor() {
        this.storageKey = 'repair_history';
    ...
    addRecord(sheetName, inspectionType, equipmentName, notes, cost = 0) {
        const record = {
            date: todayKST(),
            sheet: sheetName,
            type: inspectionType,
    ...
    // 과거 레코드에는 cost 필드가 없고 금액이 특이사항에 [수리금액:150000] 형태로만
    // 들어가 있습니다. 읽는 시점에 한 번 보정해 줍니다. (클라우드 응답에도 동일 적용)
    static normalizeCost(record) {
        if (!record) return record;
        if (typeof record.cost === 'number' && record.cost > 0) return record;

        const parsed = Number(record.cost);
        if (Number.isFinite(parsed) && parsed > 0) return { ...record, cost: parsed };

        const match = /\[수리금액:\s*(\d+)\s*\]/.exec(record.notes || '');
        return { ...record, cost: match ? parseInt(match[1], 10) : 0 };
    }

    getAllNormalized() {
        return this.history.map(RepairTracker.normalizeCost);
    }

    getByTeam(sheetName) {
        return this.history.filter(r => r.sheet === sheetName);
    }
    ...
    getTeamSummary() {
        ...
        return teams.map(t => ({
            ...t,
            total: this.history.filter(r => r.sheet === t.sheet).length,
            records: this.history.filter(r => r.sheet === t.sheet)
        }));
    }

    getEquipmentSummary(sheetName) {
        const records = this.getByTeam(sheetName);
        const map = {};
        records.forEach(r => {
            map[r.equipment] = (map[r.equipment] || 0) + 1;
        });
