...
async function submitInspection() {
    ...
    for (const equipmentName of equipmentList) {
        ...
        if (hasRepair) {
            // 로컬은 항상 기록 (오프라인 대비)
            const record = repairTracker.addRecord(
                sheetName, inspectionData.type, equipmentName,
                notesWithCost, inspectionData.repairCost
            );

            // 시트에도 올립니다. 실패해도 점검 저장 자체는 막지 않습니다.
            if (googleSheetsManager.webAppUrl) {
                try {
                    await googleSheetsManager.addRepairRecord(record);
                } catch (err) {
                    console.warn(`수리 기록 시트 저장 실패 (${equipmentName}):`, err);
                    repairSyncFailed.push(equipmentName);
                }
            }
    ...
    if (repairSyncFailed.length > 0) {
        saveResult += `\n⚠️ 수리 기록은 이 기기에만 저장되었습니다 (${repairSyncFailed.length}건).`;
    }
